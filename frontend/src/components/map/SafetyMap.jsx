...
const SafetyMap = () => {
  const { currentLocation, loading, error, isFallbackLocation } = useLocation();
  ...
  return (
    <div className="w-full rounded-lg overflow-hidden shadow-md border border-gray-200">
      ...
      {isFallbackLocation && (
        <div className="bg-yellow-50 border-b border-yellow-200 p-3 text-yellow-700 text-sm">
          <div className="flex items-start">
            ...
            <div>
              <p className="font-medium">Using approximate location</p>
              <p className="mt-1">{error || 'Your exact location is unavailable. Map features may not reflect your actual position.'}</p>
              <button 
                onClick={() => window.location.reload()} 
                className="mt-2 px-3 py-1 bg-yellow-100 hover:bg-yellow-200 rounded-md text-yellow-800 text-xs font-medium transition-colors"
              >
                Try again with real location
              </button>
            </div>
          </div>
        </div>
        ...
        >
          ...
          {currentLocation && (
            <Marker position={[currentLocation.lat, currentLocation.lng]}>
              <Popup>
                {isFallbackLocation ? "Approximate location (not your actual position)" : "Your current location"}
              </Popup>
            </Marker>
          )}
      ...
      {routeData && viewMode === 'route' && !routeLoading && (
        <div className="bg-white rounded-lg shadow-sm p-4 m-4 border-t border-gray-200">
          ...
          {/* Warning about fallback location in route info */}
          {isFallbackLocation && (
            <div className="text-yellow-600 text-sm mt-1 flex items-center">
              ...
              Route based on approximate location
            </div>
          )}
      ...
      {isochroneData && viewMode === 'isochrone' && !routeLoading && (
        <div className="bg-white rounded-lg shadow-sm p-4 m-4 border-t border-gray-200">
          ...
          {/* Warning about fallback location in isochrone info */}
          {isFallbackLocation && (
            <div className="text-yellow-600 text-sm mt-1 flex items-center">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" viewBox="0 0 20 20" fill="currentColor">
                <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
              </svg>
              Based on approximate location, not your actual position
            </div>
          )}
