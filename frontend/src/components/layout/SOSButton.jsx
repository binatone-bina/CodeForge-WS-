  ...
  const handleSOSClick = async () => {
    ...
    try {
      ...
      if (response.emailSent) {
        successDetails.push('Email alerts');
      }
      
      if (response.smsSent) {
        successDetails.push('SMS alerts');
      }
      
      if (response.callsMade) {
        const callCount = response.callDetails?.filter(c => c.success)?.length || 0;
        successDetails.push(`${callCount} voice calls`);
      }
